...
8. [SEO・アクセス解析（GTM/GA4）の設定](#8-seoアクセス解析gtmga4の設定)
...
### 不具合7：文中リンクが見た目上リンクとわからない

**症状：** `<a>`タグは存在し実際にクリックすれば機能するのに、色も下線も無く周囲の文章と同化していて、ユーザーが「リンクだと気づけない」（同意チェックボックス内のプライバシーポリシーリンク等で発覚。オーナーから「リンクの表示がない」という指摘を受けた）
**原因：** ベースCSSで`a{color:inherit;text-decoration:none}`のようなリセットをかけている場合、ナビ・ボタンなど個別にスタイルした要素は問題ないが、本文中に埋め込んだ単発のリンクは何のスタイルも持たないまま埋もれる。`:hover`時だけ色を変える指定も、タッチデバイスでは実質機能せず解決にならない
**対処：**

```css
/* 本文中リンクは通常時から視覚的に分かるようにする（hoverだけに頼らない） */
.consent-text a, .legal-link {
...
新しく本文中にリンクを追加したら、コードを書いた時点で満足せず、実際にブラウザで「リンクだと一目で分かるか」を確認する。特に同意チェックボックスのプライバシーポリシー/利用規約リンクは法的に重要な導線のため優先的に点検する。
...
## 8. SEO・アクセス解析（GTM/GA4）の設定

### 8-1. よくある罠：canonical/OGP/構造化データのドメインがプレースホルダーのまま

新規サイト制作時、`<link rel="canonical">`・`og:url`・`og:image`・schema.orgの`@id`/`url`に一時的な`https://example.com/`を入れたまま、実際の公開ドメインへの置き換えを忘れるケースがある。

**影響**：SNSシェア時にOGP画像が読み込めない／検索エンジンが誤ったURLを正規ページと判定する／構造化データが無効になりリッチリザルトが機能しない。

**チェック方法**：公開後、`view-source:`でcanonical・og:url・og:imageの値を確認し、実ドメインになっているか確認する。`sitemap.xml`・`robots.txt`内のURLも同様に確認する。

### 8-2. GA4（Googleアナリティクス4）の設置手順

1. サイトに既にGTM（Google Tag Manager）が入っているか確認する（`<script>`内に`googletagmanager.com/gtm.js?id=GTM-XXXXXXX`があるか）。
   - 入っている場合 → 8-3へ（GTM経由でGA4タグを追加する。二重にタグを埋め込まないよう、GA4の「手動でインストールする」のコードは使わない）
   - 入っていない場合 → まずGTMコンテナを新規作成し（tagmanager.google.comの「アカウントを作成」）、発行された`GTM-XXXXXXX`のスニペットを`<head>`直後と`<body>`開始直後の2箇所に設置する
2. [analytics.google.com](https://analytics.google.com)で新規プロパティを作成する（「測定を開始」）。
   - プロパティ名・タイムゾーン（日本）・通貨（JPY）を入力
   - プラットフォームは「ウェブ」を選択し、サイトURLとストリーム名を入力
   - 完了すると測定ID（`G-`から始まる文字列）が発行される

### 8-3. GTM側でのGA4タグ設定

1. [tagmanager.google.com](https://tagmanager.google.com)で該当コンテナを開く
2. 「タグ」→「新規」→タグの種類で「**Google タグ**」を選択する（「GA4 イベント」ではない。以前の「Google アナリティクス: GA4 設定」に相当する名称にGoogle側で変更されている）
3. 「タグ ID」に測定ID（`G-XXXXXXXXXX`）を入力する（同一Googleアカウント内であれば自動入力されることがある）
4. 「トリガー」で「**Initialization - All Pages**」を選択する（組み込みトリガーのため一覧の「トリガー」件数には数えられないが正常）。通常の「All Pages」でも動作するが、他のタグより先に読み込ませることがGoogleの公式推奨のため、こちらを使う
5. タグに分かりやすい名前を付けて保存する
6. 右上「公開」→ バージョン名・説明を入力（省略可）→「公開」
7. 確認：Googleアナリティクスの「レポート」→「リアルタイム」を開いた状態で実際にサイトへアクセスし、データが表示されることを確認する

### 8-4. サイトが複数ある場合の注意

- GA4プロパティ・GTMコンテナは**サイト（ドメイン）ごとに別々に作成する**。1つのGA4プロパティ/GTMコンテナを複数の異なるサイトで使い回さない
- 既存のGTMがないシンプルな静的1ページサイトの場合、GTMを新規導入せず、GA4の「手動でインストールする」のgtag.jsスニペットを`<head>`に直接貼り付ける方が簡潔なこともある（今後タグを追加する予定がなければ）

### 8-5. GA4のリアルタイムに反映されないときの切り分け方

タグ設置後にGA4のリアルタイムレポートに数字が出ない場合、以下の順で切り分ける。

1. GTMの「プレビュー」でTag Assistantに接続し、対象のタグが「配信されたタグ」に出るか確認する。出ていればGTM側のタグ発火自体は成功している
2. ブラウザのDevTools → Networkタブで「collect」を検索し、**シークレットウィンドウ**で対象ページを再読み込みする。`collect?...tid=G-XXXXXXXXXX...`という行がStatus **204**で出れば、実際の送信自体は成功している
   - **通常ウィンドウでは出ないのにシークレットウィンドウでは出る場合、広告ブロッカー系の拡張機能が原因**。gtag.jsライブラリ本体（`js?id=G-XXXXXXXXXX`）は読み込めるのに、計測用のcollectリクエストだけをピンポイントで止めるタイプの拡張機能があるため紛らわしい
3. ここまで確認できてもリアルタイムに反映されない場合、GA4の「管理」→「データストリーム」で測定IDがそのプロパティ・ストリームと完全に一致しているか確認する
4. 「データ収集が有効になっていません」という警告は、作成直後のストリームでは一律表示されることがあるため、上記1〜3が確認できていれば深刻な問題ではない可能性が高い。反映まで数十分〜半日程度のタイムラグが出ることがある
