...
- **多段ケーキのサイズは段数ごとに連動して自動的に決まる**（2026-09-05確定）：オーナー指示により、上の段は下の段より必ず2号小さくする方針にした。実在しない小サイズ（2号・1号）の代わりに、2段目の下限は「セルクル」固定、3段目の下限は「たぬきちケーキ／カットケーキ（内容はご予約後にご相談）／セルクル」の3択（お客様が選ぶ）にする。有効な組み合わせは2段:(4号,セルクル)/(5号,3号)/(6号,4号)/(7号,5号)、3段:(5号,3号,特別3択)/(7号,5号,3号)のみで、`member.html`の`renderTierSizeSelects()`が1段目（お客様が選ぶ唯一の段）の選択に応じて上の段を自動計算・selectをdisabledで表示する方式で実装（`TWO_TIER_PARTNER`・`THREE_TIER_MID_PARTNER`・`THREE_TIER_TOP_FIXED`定数）。新設した「3号」サイズの価格は¥3,800（オーナー指定、`admin.html`のサイズ別価格パネルに`cfPrice3`を追加、ライブFirebase`koimariContent/cakeSizePrices`にも反映が必要 — コードのDEFAULT値だけでは足りないので、admin.html「商品管理」→サイズ別価格で3号欄に3800と入力し保存すること）。たぬきちケーキ／カットケーキ／セルクルは個別価格のため自動計算対象外とし、含まれる場合は`data.priceNeedsConsult`フラグを立てて「料金は追ってお電話でご案内」表示に切り替える（中途半端な合計金額を見せないため）
