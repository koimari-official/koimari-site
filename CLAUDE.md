...
- **ケーキの種類は写真選択式（タブ切り替え、予約種別に連動）**（2026-07-12変更、同日タブ化・種別連動化）：固定のプルダウンではなく、`siteImages.categories`の`img`+`gallery`写真を、カテゴリごとのタブ（`#cakePhotoTabs`）で切り替えながら4列グリッド（`#cakePhotoGrid`）で表示し、クリックで選択する方式。**表示されるタブは種別ラジオの選択に連動**（`CAKE_PHOTO_CATEGORY_MAP`）：デコレーションケーキ予約/イベントケーキ予約→「デコレーションケーキ」「ロールケーキ」、焼き菓子予約→「焼き菓子」「ギフト」。あわせて項目ラベルも`CAKE_PHOTO_FIELD_LABELS`に従い「ケーキの種類」⇔「商品の種類」に切り替わる。種別を切り替えた際、選択済みの写真が新カテゴリに含まれない場合は選択を自動リセットする。タブ切り替え自体では選択状態（`selectedCakePhotoSrc`）は保持される。選択結果は非表示input `cakeType`（カテゴリ名）と`cakeTypeImage`（画像URL）に格納され、admin.htmlの予約一覧にサムネイル表示される。選択肢を増やすには管理画面「カテゴリ画像」タブで該当カテゴリにギャラリー写真を追加すればよい（コード変更不要）。対象カテゴリの組み合わせ自体を増やす場合は`CAKE_PHOTO_CATEGORY_MAP`・`CAKE_PHOTO_FIELD_LABELS`・`ALL_CAKE_PHOTO_CATEGORIES`の3箇所を編集する
