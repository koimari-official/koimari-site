...
### 中優先（2026-07-12昇格）
- [ ] **予約時のオンライン決済（Square）連携**：注文内容確認画面（`#confirmScreen`）の「この内容で送信する」の後に決済ステップを追加する構想。クレカ・QRコード決済（PayPay等）・電子マネーを一括カバーできる決済代行業者としてSquareをオーナーが選定（2026-07-12）。
  - **⚠️ 着手前提条件（オーナー対応待ち）**：(1) Squareビジネスアカウント開設 (2) 本人確認・事業者審査完了（銀行口座登録） (3) Square Developer DashboardでApplication ID・Location IDを発行。この3点が揃うまでコード実装は着手できない
  - **技術メモ**：Squareでカード情報を安全に処理するには決済確定処理をサーバー側（秘密鍵を露出させない場所）で行う必要があり、現状Firebaseのみで構成されている本サイトにFirebase Cloud Functions（Blazeプランへの切り替えが必要）等の裏側の仕組みを新設することになる見込み。着手時に改めて設計案を提示する
