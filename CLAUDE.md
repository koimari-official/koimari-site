...
- **予約締切は「2営業日前」（定休日・臨時休業を除く）**（2026-09-03変更、従来は1営業日前）：`member.html`で`koimariContent/holidays`（管理画面「営業日設定」と同じデータ、`weeklyClosed`=定休曜日の配列、`extraClosed`=臨時休業日の配列）を購読し、`computeOrderDeadline()`が定休日・臨時休業を除いた直近2営業日分を遡って締切日を計算する（単純な「2日前から1日ずつ戻る」ではなく、営業日を2日分カウントするロジック。例: 火曜引き取りの場合、月曜定休を挟むため締切は土曜になる）。あわせて引き取り希望日自体が定休日の場合もエラーにするチェックを追加。terms.html・tokushoho.html・faq.html・admin.html（DEFAULT_FAQ）・mothers-day.html・functions/index.js（`RESERVE_CARD_REPLY_TEXT`・AIシステムプロンプト）の記載も合わせて更新済み。引き取り希望日欄に`index.html#bcal`（営業日カレンダー）を別タブで開くリンク「定休日カレンダーをチェック」も追加
