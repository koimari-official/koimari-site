...
- **予約完了メール（お客様宛の自動返信）は完全無料のGmail SMTPで送信**（2026-09-04導入）：Formspree（`reservations`向け）は店舗宛の通知専用で、お客様への自動返信（オートレスポンス）はFormspreeの有料プラン（Professional以上、月額$30〜）でしか使えないためWebSearchで確認した上で別経路を採用。`functions/index.js`の`sendCustomerReservationEmail`（`/reservations/{pushId}`のonValueCreatedトリガー、`notifyStaffOnNewReservation`と同じ発火点）が`nodemailer`でGmail SMTP（アプリパスワード認証）から`data.email`宛に送信する。本文は`buildCustomerConfirmationEmailText()`で生成し、「スタッフより確認のお電話を差し上げます」の一文を含む。**Secretの`GMAIL_USER`／`GMAIL_APP_PASSWORD`は未設定（要オーナー作業）**：`noritate.official@gmail.com`（Formspreeで既に使用中の店舗用アドレス）の2段階認証を有効化した上でアプリパスワードを発行し、`firebase functions:secrets:set GMAIL_USER` / `GMAIL_APP_PASSWORD --project koimari-tasting`で設定してから`firebase deploy --only functions:sendCustomerReservationEmail`でデプロイする。Secret未設定の間はこの関数はエラーログを出して終了するだけで、既存の予約保存・スタッフ通知（`notifyStaffOnNewReservation`）には影響しない設計。あわせてmember.htmlの予約完了画面の文言に「予約完了をメールでお送りしますのでご確認ください」を追加済み。**2026-09-05追記**：オーナーがSecretを設定・`firebase functions:secrets:set`実行済み、`sendCustomerReservationEmail`のデプロイも完了（Firebase CLIの「stale version」自動再デプロイ機能を利用）
- **2段ケーキのサイズは1段目の選択に連動して自動的に決まる／3段は自動選択の対象外**（2026-09-05確定、同日中に方針修正あり）：オーナー指示により、上の段は下の段より必ず2号小さくする方針だったが、「3号は基本予約では受けない」「3段の細かい組み合わせはスタッフが電話で確認する」との追加指示を受け、最終的に**2段ケーキのみ**を自動選択の対象とした（**3段の商品自体をフォームから削除**）。有効な組み合わせは(4号,セルクルまたはカットケーキ[たぬきち含む])/(6号,4号)/(7号,5号)の3通りのみ（5号を1段目にする組み合わせ・3号サイズ自体は廃止）。`member.html`の`renderTierSizeSelects()`が1段目（お客様が選ぶ唯一の段）の選択に応じて2段目を自動計算・selectをdisabledで表示する方式で実装（`TWO_TIER_PARTNER`・`TWO_TIER_SPECIAL_BOTTOM`・`SPECIAL_TOP_OPTIONS`定数）。1段目が4号の場合のみ2段目は「セルクル／カットケーキ（たぬきち含む・内容はご予約後にご相談）」からお客様が選ぶ。これらは個別価格のため自動計算対象外とし、含まれる場合は`data.priceNeedsConsult`フラグを立てて「料金は追ってお電話でご案内」表示に切り替える（中途半端な合計金額を見せないため）。**3号サイズ・その価格入力欄（admin.html `cfPrice3`）は追加後に撤回済み**、3段を希望するお客様は電話・LINEチャットで直接相談してもらう運用
- **独自スタンプカード機能は完全廃止**（2026-09-08決定）：`member.html`の`stampSection`・`renderStamps()`・`koimariContent/loyaltyConfig`連動、`admin.html`「LINE会員管理」パネルのON/OFFトグル・スタンプ増減ボタン・CSV出力のスタンプ列をすべて削除済み。理由：オーナーが「管理できない」（施策担当不在で運用が回らない）ため、代わりにLINE公式アカウント自体の「ショップカード」機能（LINE側が提供する来店ポイント機能）に一本化する方針。index.html・reservation.htmlの会員登録案内文からも「スタンプカード」の記載を削除済み。**ライブFirebaseの`koimariContent/loyaltyConfig`・各会員の`lineMembers/{uid}/stamps`フィールドはコードから参照されなくなったが、データ自体は削除していない**（実害のない残骸のため、削除する場合は別途対応）。LINEのショップカード設定自体はLINE Official Account Manager側の操作でありこのリポジトリの管理対象外
- **リッチメニュー「よくある質問」タイルを「スタンプカード」に差し替え**（2026-09-08、`RICHMENU_VERSION`を`2026-09-08-stampcard-v1`に更新）：ショップカードのURLがまだ確定していないため、タップ時は`STAMP_CARD_TRIGGER_TEXT`のメッセージアクション→`STAMP_CARD_REPLY_TEXT`（準備中の案内、`COUPON_TRIGGER_TEXT`と同じ確定応答パターン）を返す仕組みにした。**ショップカードのURLが決まり次第、`RICHMENU_AREAS`の該当actionを`{type:"uri", uri:"<実URL>"}`に変更し、`RICHMENU_VERSION`を再度更新してデプロイすること**（要オーナー確認事項として残っている）。アイコンは新規`openmoji/stampcard.svg`（カード+スタンプ5個の自作アイコン）。画像の差し替えは全体再書き出しではなく、該当タイルだけを単独HTMLでレンダリングしPILで部分合成する方法を採用（`richmenu-src/README.md`の「1タイルだけを差し替える場合」参照、他5タイルへの影響ゼロを担保）。FAQページ自体（faq.html）は削除しておらず、AIチャットのFAQ知識注入（`buildFaqKnowledgeText`）もそのまま維持——お客様がチャットで質問すればAIが引き続き正確に回答する
- **生チョコクリーム（CF）は生クリーム（F）より段数に関わらず一律+200円**（2026-09-05確定）：2026-09-04時点では「生クリーム／生チョコクリームのいずれを選んでも同一料金」だったが、実際の価格表ではCFがFより+200円高いことが判明し方針変更。`member.html`の`currentChocoCreamSurcharge`（Firebase `koimariContent/cakeChocoCreamSurcharge`、admin.htmlの`cfChocoCreamSurcharge`で編集）を新設し、`creamType === "生チョコクリーム"`の場合のみ段数・サイズに関わらず一律加算する。確認画面・LINEチャットサマリー・Formspree通知メールのいずれにも「（+¥200）」の表示を追加済み
- **引き取り希望時間は営業終了30分前までが選択肢の上限**（既存実装・2026-09-05確認）：`member.html`の`timeOptionsForDate()`が日祝19:00・火〜土20:00の閉店時刻から30分引いた時刻までを30分刻みで生成する。新規実装ではなく既に組み込み済みだった
- **引き取り希望日とGoogleビジネスプロフィールの営業日連動は「手動同期」運用**（2026-09-05決定）：Google Places API連携（有料の可能性・開発工数増）ではなく、admin.html「営業日設定」を唯一の正データとし、変更のたびにGoogleビジネスプロフィール側にも同じ内容を手動反映する運用にした。admin.htmlの営業日設定パネルにその旨の注意書きを追加済み
