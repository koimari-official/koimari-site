...
### 高優先
- [ ] 【最重要・未着手】ヒーロー画像・カテゴリ画像・おすすめ商品・体験プログラムの画像が、Firebaseではなく管理者自身のブラウザのlocalStorage（`koimari_images`キー、admin.html L702の`STORAGE_KEY`）に保存されている。index.html側の`loadImages()`（L870）も自分のブラウザのlocalStorageを見に行く実装のため、**管理画面で画像をアップロード・保存しても、実際の来店客には一切反映されておらず、常にコード埋め込みのデフォルト画像が表示され続けている可能性が高い**（2026-07-09発見）。「容量超過」エラーの根本原因でもある（localStorage全体で約5MBの上限があり、複数画像をひとまとめのJSONで保存する設計のため超過しやすい）。正しい修正は予約/体験応募/法人相談と同じFirebase移行だが、index.html側の同期読み込みロジック変更を伴い本番トップページを壊すリスクがあるため、オーナー了承のもと今回は着手を見送り、次回改めて集中して対応する
