...
- `openmoji/*.svg` — 各アイコンの元SVG。ご予約=`cake.svg`（自作、キャンドル+苺付き）、
  ...
  オーナー提供の参考画像を元に作成）、スタンプカード=`stampcard.svg`（自作、カード+スタンプ5個、
  2026-09-08に「よくある質問」タイルから差し替え）。`ticket.svg`/`ticket2.svg`は使用していない旧案
  （[OpenMoji](https://openmoji.org)由来、CC BY-SA 4.0）

⚠️ **`richmenu-source.html`はアイコン部分がプレースホルダー（`__ICON_CAKE__`等）のままで、
実際に配信中の画像を再現するには本来は別のスクリプトでSVGを差し込む必要があった（この運用は
記録が失われていた）。1タイルだけを差し替える場合は、下記「1タイルだけを差し替える場合」の
方法（切り抜き→部分合成）を使うと、他5タイルに一切影響を与えずに安全に変更できる。**

## 1タイルだけを差し替える場合（推奨・安全な方法）

全体を`richmenu-source.html`から再書き出しすると、プレースホルダー未解決の問題や
微妙なフォント読み込みタイミング差で他のタイルまで意図せず変わるリスクがある。
1タイルのアイコン・ラベルだけ変える場合は、そのタイルだけを単独の833×843px HTMLとして
書き出し→Playwrightでスクリーンショット→既存の`richmenu-main.jpg`に該当座標だけPIL等で
上書き合成する方が安全（2026-09-08、スタンプカードタイル追加時に採用した方法）。
`.tile--dark`の場合の単独HTML例：背景に`itachoco_block.png`を相対パスで参照し、
`.glow`・`.icon-slot`・`.sil`・`.jp`のCSSは`richmenu-source.html`と同じ値を使う。
